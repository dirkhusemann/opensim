...
if __name__ == '__main__':
    ...
    if not config.has_section('opensim'):
        logging.error('[expander] %s: config file "%s" lacks an [opensim] section',
                      options.template, options.config)

    expansions = dict()
    for k in config.options('opensim'):
        expansions[k] = config.get('opensim', k)
